...
/*  Copyright 2006-2019  Hans Matzen  (email : webmaster at tuxlog dot de)
...
{ 
	...
	if ( ($_SERVER['QUERY_STRING']=="page=wp-forecast-admin.php") && (isset($_POST['info_update']) ))
	{
		for ($i=0;$i<$count;$i++) {
...
function wpf_admin_form($wpfcid='A',$widgetcall=0) 
{
    ...
    if ( function_exists("is_multisite") && is_multisite() && $blog_id!=1 && ! wpf_get_option('wpf_sa_allowed'))
	...
	if ($widgetcall==0) {
		...
		if (isset($_POST['wpf_global_settings_update'])) {
			...
			// update delopt
			$wpf_delopt = (array_key_exists('wp-forecast-delopt', $_POST)?(int) ($_POST['wp-forecast-delopt'] == "on"): false);
			...
			wpf_update_option('wp-forecast-ipstackapikey', $_POST['wp-forecast-ipstackapikey']);
			
			// update loadcss
			$wpf_loadcss = (array_key_exists('wp-forecast-loadcss', $_POST)?(int) ($_POST['wp-forecast-loadcss'] == "on"): false);
    ...
    $out .= "<h2>" . __('Global Settings',"wp-forecast_".$locale)."</h2>";
    $out .= "<form name='options' id='options' method='post' action='#'>";
    $out .= '<table style="width:80%;"><tr><td>'.__('How many wp-forecast widgets would you like?',"wp-forecast_".$locale)."</td>";
    ...
    $out .= "<td>".__('Timeout for weatherprovider connections (secs.)?',"wp-forecast_".$locale).":</td>";
    ...
    $out .= "<td>".__('Delete options during plugin deactivation?',"wp-forecast_".$locale)."</td>";
    ...
    $out .= "<tr><td><a href='http://www.ipstack.com' target='_blank'>".__('API key to use location from ipstack.com ',"wp-forecast_".$locale).":</a></td>";
    ...
    $out .= "<td>".__('Do not load default CSS',"wp-forecast_".$locale)."</td>";
	...
	$out .= "<div class='submit'><input class='button-primary' type='submit' name='wpf_global_settings_update' value='".__('Update global options',"wp-forecast_".$locale)." »' /></div>";
    $out .= "</form></div>\n"; 
    ...
    if ($count > 1) {
		...
		$out .="<div>".__('Available widgets',"wp-forecast_".$locale).": ";
		...
		$out .=__('Select widget',"wp-forecast_".$locale)." »\" /></span></div></form>\n";
    }
...
function wpf_sub_admin_form($wpfcid,$widgetcall) {
	...
	if (isset($_POST['info_update'])) {
		...
		$wpf_admin_fields = array(
			'service', 'apikey1','location','loclongitude','loclatitude', 'visitorlocation','locname','location','refresh','provider',
			'csssprites','windicon','pdforecast','pdfirstday','timeoffset','currtime','wpf_language','windunit','metric',
			'ouv_safetime', 'ouv_ozone', 'ouv_uvmax','ouv_uv','ouv_apikey'
		);
		...
		foreach ($wpf_admin_fields as $k) {
			if (!$ismulti or isset($allowed["ue_".$k])) {
				$av[$k] = $_POST[$k];
				$upflag=true;
				
				// update latitude and longitude if location is changed, update it in POST !!!
				// make sure location is before loclangitude and loclatitude in array wpf_Admin_fields
				//
				if ($k == 'location' && $av['location'] != "") {
					$coordxml = accu_get_weather($av['ACCU_BASE_URI'], $av['location'], 1);
					$coord = new SimpleXMLElement($coordxml);
					$_POST["loclatitude"]  = (string) $coord->local[0]->lat;
					$_POST["loclongitude"] = (string) $coord->local[0]->lon;
				}
			}
		}
		...
		if ($av['ouv_uv']=="") $av['ouv_uv']="0";
     	if ($av['ouv_uvmax']=="") $av['ouv_uvmax']="0";
     	if ($av['ouv_ozone']=="") $av['ouv_ozone']="0";
     	if ($av['ouv_safetime']=="") $av['ouv_safetime']="0";
		...
		$do = array('d_c_icon','d_c_time','d_c_short','d_c_temp','d_c_real','d_c_press','d_c_humid','d_c_wind','d_c_sunrise',
					'd_c_sunset','d_d_icon','d_d_short','d_d_temp','d_d_wind','d_n_icon','d_n_short','d_n_temp','d_n_wind',
					'd_c_date','d_d_date','d_n_date','d_c_copyright','d_c_wgusts','d_d_wgusts','d_n_wgusts','d_c_accuweather',
					'd_c_aw_newwindow','d_c_uvindex','d_d_maxuv','d_n_maxuv','d_c_precipe','d_d_precipe','d_n_precipe');
		foreach ($do as $i) {
			if (!isset($_POST[$i]) || $_POST[$i]=="")
				$_POST["$i"]="0";
		}
	...
	if (isset($_POST['set_loc'])) {
		if (isset($_POST['new_loc'] )) $av['location'] = $_POST["new_loc"];
		if (isset($_POST['provider'])) $av['service']  = $_POST['provider'];
	}
?>
	 ...
	 <div style="float: left; width: 49%">
         ...
         <select name="service" id="service" size="1" onchange="apifields(document.woptions.service.value);">
           ...
           <option value="darksky" <?php if ($av['service']=="darksky") echo "selected=\"selected\""?>><?php echo __('Dark Sky',"wp-forecast_".$locale)?></option>
	 </select>&nbsp;
         ...
         <!--
         <p><?php echo __('API-Key',"wp-forecast_".$locale)?>:
	...
	<a href="<?php echo plugin_dir_url( __FILE__ ); ?>/wp-forecast-search.php?height=600&amp;width=800&amp;wpfcid=<?php echo $wpfcid;?>" class="thickbox" title="">
		<img alt="Search Icon" src="<?php echo plugin_dir_url( __FILE__ ); ?>/Searchicon16x16.png" /></a>
		&nbsp;&nbsp;&nbsp;<input type="checkbox" name="visitorlocation" id="visitorlocation" value="1" <?php if (isset($av['visitorlocation']) && $av['visitorlocation']=="1") echo "checked=\"checked\""?> onchange="vlocfields_update();" /><b><?php echo __('Use visitors location',"wp-forecast_".$locale)?></b>
	 </p>

	<p><b><?php echo __('Latitude',"wp-forecast_".$locale)?>:</b>
         <input name="loclatitude" id="loclatitude" type="text" size="15" maxlength="10" value="<?php if (isset($av['loclatitude'])) echo $av['loclatitude']; ?>" />
         
     <b><?php echo __('Longitude',"wp-forecast_".$locale)?>:</b>
         <input name="loclongitude" id="loclongitude" type="text" size="15" maxlength="10" value="<?php if (isset($av['loclongitude'])) echo $av['loclongitude']; ?>" /></p>
    
     <?php if (isset($_POST['set_loc'])) { ?>
	       <p><b><?php echo __('Press Update options to save new location and update coordinates.',"wp-forecast_".$locale)?></b></p>
     <?php } ?>
	 ...
	 <p><input type="checkbox" name="currtime" id="currtime" value="1" <?php if ($av['currtime']=="1") echo "checked=\"checked\""?> /> <b><?php echo __('Use current time',"wp-forecast_".$locale)?></b>
												 / <b><?php echo __('Time-Offset',"wp-forecast_".$locale)?> :</b> <input type="text" name="timeoffset" id="timeoffset" size="5" maxlength="5" value="<?php echo (isset($av['timeoffset'])?$av['timeoffset'] :"");?>" /> <b><?php echo __('minutes',"wp-forecast_".$locale);?></b> 
         </p>
	 ...
	 <p>
         <b><?php echo __('Language',"wp-forecast_".$locale)?>: </b><select name="wpf_language" id="wpf_language" size="1">
            ...
            <option value="hr_HR" <?php if ($av['wpf_language']=="hr_HR") echo "selected=\"selected\""?>>croatian</option>
...
<b><?php echo __('Forecast',"wp-forecast_".$locale)?></b>
       ...
       <?php 
			// OpenUV parameters
			echo "<div>";
			echo  "<b><a href='http://www.openuv.io' target='_blank'>" . __('Use UV-Data from openuv.io',"wp-forecast_".$locale) . ":</a></b>&nbsp;&nbsp;&nbsp;";
			
			echo __('OpenUV - API-Key',"wp-forecast_".$locale);
			echo ':<input name="ouv_apikey" id="ouv_apikey" type="text" size="40" maxlength="80" value="' . (isset($av['ouv_apikey'])?$av['ouv_apikey']:""). '" />&nbsp;&nbsp;&nbsp;';
			
			echo '<input type="checkbox" name="ouv_uv" id="oav_uv" value="1"';
			if (isset($av['ouv_uv']) && $av['ouv_uv']=="1") echo "checked=\"checked\"";
			echo "/>";
			echo '<label for="ouv_uv">' . __('Show UV-Index', "wp-forecast_".$locale) . '&nbsp;&nbsp;&nbsp;</label>';
			
			echo '<input type="checkbox" name="ouv_uvmax" id="oav_uvmax" value="1"';
			if (isset($av['ouv_uvmax']) && $av['ouv_uvmax']=="1") echo "checked=\"checked\"";
			echo "/>";
			echo '<label for="ouv_uv">' . __('Show max. UV-Index', "wp-forecast_".$locale) . '&nbsp;&nbsp;&nbsp;</label>';
			
			echo '<input type="checkbox" name="ouv_ozone" id="oav_ozone" value="1"';
			if (isset($av['ouv_ozone']) && $av['ouv_ozone']=="1") echo "checked=\"checked\"";
			echo "/>";
			echo '<label for="ouv_uv">' . __('Show Ozone', "wp-forecast_".$locale) . '&nbsp;&nbsp;&nbsp;</label>';
			
			echo '<input type="checkbox" name="ouv_safetime" id="oav_safetime" value="1"';
			if (isset($av['ouv_safetime']) && $av['ouv_safetime']=="1") echo "checked=\"checked\"";
			echo "/>";
			echo '<label for="ouv_uv">' . __('Show safe exposure time as tooltip',"wp-forecast_".$locale) . '&nbsp;&nbsp;&nbsp;</label>';
