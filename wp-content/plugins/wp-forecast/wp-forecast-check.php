...
/*  Copyright 2009-2019  Hans Matzen  (email : webmaster at tuxlog dot de)
...
if (!empty($_POST)) {
    ...
    if ( $_POST['wprovider'] == "Dark Sky") {
		if ( !isset($wpf_vars['apikey1'])) {
			$out .= "You have to set the API-Key to test WeatherBug.";
			echo $out;
			die();
		}
		$url = $wpf_vars["DARKSKY_BASE_URI"] . $wpf_vars['apikey1'] . "/49.68,8.98/?exclude=minutely,hourly&units=si&lang=de";
    }
...
$out .= "<option value='Dark Sky'>Dark Sky</option>\n";
$out .= "</select></td>\n";
