...
if (!function_exists('accu_xml_parser')) {
	...
	function accu_start_element($parser, $name, $attribute) {
		...
		if (isset($wpf_path_table[$wpf_pstack])) {
			...
			if ($wpf_fc_daynumber != "0" and $wpf_fc_daynumber != "") $wpf_go_ahead = $wpf_go_ahead . "_" . $wpf_fc_daynumber;
		}
		// for forecast days
		
		if ($wpf_pstack == "/ADC_DATABASE/FORECAST/DAY") $wpf_fc_daynumber = $attribute["NUMBER"];
		// for current sun rise/set
		
		if ($wpf_pstack == "/ADC_DATABASE/PLANETS/SUN") $wpf_weather["sun"] = $attribute["RISE"] . " " . $attribute["SET"];
		// for current time daylightsavings
		
		if ($wpf_pstack == "/ADC_DATABASE/LOCAL/GMTDIFF") {
			
			if ($attribute["DAYLIGHTSAVINGS"] != "") $wpf_weather['gmtdiffdls'] = $attribute["DAYLIGHTSAVINGS"];
			else $wpf_weather['gmtdiffdls'] = 0;
		}
		// for current uvindex
		
		if ($wpf_pstack == "/ADC_DATABASE/CURRENTCONDITIONS/UVINDEX") $wpf_weather['uvindex'] = $attribute["INDEX"];
	}
	...
	function accu_end_element($parser, $name) {
		...
		if ($name == "DAY") $wpf_fc_daynumber = 0;
	}
	...
	function accu_xml_parser($xmlstring) {
		...
		if (!xml_parse($parser, $xmlstring, true)) {
			...
			$xmlerror = "XML Fehler: " . xml_error_string(xml_get_error_code($parser)) . " in Zeile " . xml_get_current_line_number($parser);
		}
	...
	function accu_get_locations($xml) {
		...
		if (!xml_parse($parser, $xml, true)) {
			...
			die("XML Fehler: " . xml_error_string(xml_get_error_code($parser)) . " in Zeile " . xml_get_current_line_number($parser));
		}
		// Vom XML-Parser belegten Speicher freigeben
		xml_parser_free($parser);
	}
	...
	function accu_forecast_data($wpfcid = "A", $language_override = null) {
		...
		if (!isset($w['failure'])) {
			...
			if ($w['gmtdiffdls'] == 1) $ct+= 3600; // time with daylightsavings
			...
			if (strlen($press) == 3 and substr($press, 0, 1) == "1") $press = $press * 10;
